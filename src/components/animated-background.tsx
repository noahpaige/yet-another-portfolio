...
const generateBlobs = (): BlobData[] => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let pairIdx = 0; pairIdx < colorPairs.length - 1; pairIdx++) {
      const [curC1, curC2] = colorPairs[pairIdx];
      const [nextC1, nextC2] = colorPairs[pairIdx + 1];
      ...
      for (let j = 0; j < colorPairs.length; j++) {
        const localFactor = (j / colorPairs.length + i / n) / 2;
        colorSteps.push({
          a: hslToString(
            interpolateHSL(
              colorA1,
...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({
  ...
  const stepCount = blobs[0].colors.length;
  const stepProgress = useTransform(
    scrollYProgress,
    [0, 1],
    [0, stepCount - 1]
  );
  const colorIndex = useTransform(stepProgress, (v) => Math.floor(v));
  const nextColorIndex = useTransform(stepProgress, (v) =>
    Math.min(Math.ceil(v), stepCount - 1)
  );
  const mixFactor = useTransform(stepProgress, (v) => v % 1);

  const y = useTransform(scrollYProgress, [0, 1], [80, 20]);

  // const blobColors: [{ a: string; b: string }] = useEffect(
  //   () => {
  //     return blobs.map((blob) => blob.colors[colorIndex.get()]);
  //   },
  //   [blobs, colorIndex, y]
  // );
      ...
      <defs>
        {blobs.map((blob, index) => {
          const aColor = useTransform(
            [colorIndex, nextColorIndex, mixFactor],
            (values: number[]) => {
              const [aIdx, bIdx, factor] = values;
          ...
          return (
            <linearGradient
              ...
              y2="0"
            >
              <motion.stop offset="0%" stopColor={aColor} />
              <motion.stop offset="100%" stopColor={bColor} />
            </linearGradient>
          );
        })}
      </defs>
